...
void rotate(int x,int &k) //x���ӽ�k�ķ�����ת 
{
...
void splay(int x,int &k) //��x��ת��k��λ�� 
{
	while(x!=k)
	{
		int y=fa[x],z=fa[y];
...
int find(int x,int rk) //Ѱ��x���ҵ�rk��Ԫ�� 
{
	pushdown(x);
	int l=c[x][0],r=c[x][1];
...
int split(int k,int tot) //�Ѵ�k��ʼ��tot��ת�Ƶ���kΪ���������� 
{
...
void query(int k,int tot) //�����k��ʼ����tot��֮�� 
{
	int x=split(k,tot);
	printf("%d\n",sum[x]);
}

void modify(int k,int tot,int val) //��K��ʼ����tot����Ϊval 
{
...
void rever(int k,int tot) //��k��ʼ��תK�� 
{
	int x=split(k,tot),y=fa[x];
	if(!tag[x])
	{
...
void erase(int k,int tot) //��k��ʼ����tot�� 
{
	int x=split(k,tot),y=fa[x];
	rec(x);c[y][0]=0;
	update(y);update(fa[y]);
...
void build(int l,int r,int f) //����һ��splay����l��r������f 
{
	if(l>r)return;
...
void insert(int k,int tot) //�ڵ�k��Ԫ�غ������tot��Ԫ�� 
{
